package io.yasiekz.github.eventsurcingexample.domain.aggregate.payment;

import io.yasiekz.github.eventsurcingexample.domain.event.Event;
import java.time.LocalDateTime;
import java.util.UUID;
import lombok.Builder;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.ToString;

@Getter
@EqualsAndHashCode(callSuper = true)
@ToString
public class PspAcceptedEvent extends Event<Payment> {

    private final PspResult pspResult;

    @Builder(setterPrefix = "with")
    public PspAcceptedEvent(final UUID id, final UUID aggregateId,
        final PspResult pspResult, final LocalDateTime occuredAt) {
        super(id, aggregateId, occuredAt);
        this.pspResult = pspResult;
    }

    @Override
    public void apply(final Payment aggregate) {
        aggregate.apply(this);
    }
}
